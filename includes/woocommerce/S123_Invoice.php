...
class S123_Invoice extends S123_BaseController
{
    ...
    */
    ...
    {
        $client = [];
        $clientCode = $this->order->get_meta('_billing_company_code', true);
        ...
        } else if ($this->order->get_meta('_billing_company_name', true)) {
            $client["name"] = $this->order->get_meta('_billing_company_name', true);
        } else {
        ...
        $clientVat = $this->order->get_meta('_billing_company_vat_code', true);
        if ($clientVat) {
    ...
    */
    private function s123_invoicePayments(): array
    {
        ...
        if ($this->s123_get_option('create_unpaid_invoices', false)) {
    ...
    */
    ...
    public function s123_generateInvoice(): array
    {
        ...
        $invoiceTypeMeta = get_post_meta($this->order->get_id(), '_i123_invoice_type', true);

        $isDefaultAdvanced = $this->s123_get_option('default_advanced_invoices', false);

        if (!empty($invoiceTypeMeta)) {
            $invoiceType = $invoiceTypeMeta;
        } else {
            $invoiceType = $isDefaultAdvanced ? S123_BaseController::I123_ADVANCED_INVOICE : S123_BaseController::I123_SIMPLE_INVOICE;
            update_post_meta($this->order->get_id(), '_i123_invoice_type', $invoiceType);
        }

        $seriesId = $invoiceType === S123_BaseController::I123_SIMPLE_INVOICE ? $this->settings['series_id'] : $this->settings['advanced_series_id'];
        ...
        return [
            ...
            'language' => get_post_meta($this->order->get_id(), '_i123_invoice_language', true),
            'template_id' => $this->settings['template_id'],
            'use_warehouse_supplies' => $this->settings['sync_warehouse'] ?? false,
            'save_client' => $this->settings['save_client'] ?? false,
